...
| app/providers | Configuration, lifecycle subscription, AppState refresh, authentication state and action injection | [SessionProvider](../../apps/mobile/src/app/providers/SessionProvider.tsx) |
| app/navigation | Conditional registration of sign-in, readiness, and authenticated screens; per-user Relay scope | [AppNavigator](../../apps/mobile/src/app/navigation/AppNavigator.tsx) |
| Sign-in route | Login layout and binding session actions/errors to the form | [SignInPage](../../apps/mobile/src/routes/SignInPage.tsx) |
| Session entity | Public session-state union, available actions, and authenticated-screen accessor | [session-context](../../apps/mobile/src/entities/session/session-context.tsx) |
...
SessionProvider reads the sign-in session through useSyncExternalStore. The session exposes a cached snapshot and a shared subscription: the first subscriber attaches the authentication lifecycle and injected native AppState refresh adapter; the last unsubscribe releases both. The native adapter uses an AbortController to ignore refresh failures after cleanup. External synchronization remains at the subscription boundary; the provider has no useEffect or duplicate React session state. AppNavigator selects mutually exclusive pending, signed-out, and signed-in screen groups from that state; login is registered alongside the other screens. Only the signed-in tree receives the user-keyed Relay provider, covering both Tabs and Account. Account connection runs from startup/auth events or explicit sign-in; it is not triggered by an effect watching React state. Each page exports its navigation options; AppNavigator.tsx centrally declares the screen registry and every stack using React Navigation static configuration. Tab definitions and their default ordering live beside the stack registry. Platform-specific AppTabs adapters render the supplied definition array, using stable route names and separate standard/iOS options. A future experiment chooses its array at the caller; the adapters do not own assignment or hardcode tab destinations. Removing a tab does not promise preservation of its navigation state. Home owns session lookup and account navigation, while `entities/user` renders the avatar button from props. Static headers need no effect; Search retains native search-bar synchronization with page state.
...
The session-provider/external-store refactor passes 70 mobile tests and type checking. Store tests cover cached snapshots, shared subscriptions, idempotent cleanup, stale callbacks or promises after resubscription, and in-flight logout deduplication across resubscription. The restarted iOS simulator displays Home with the circular account avatar, English interface copy, and no persistent email/logout strip. Account tap/back navigation, actual provider sign-in, and native logout behavior have not been reverified for this revision.
